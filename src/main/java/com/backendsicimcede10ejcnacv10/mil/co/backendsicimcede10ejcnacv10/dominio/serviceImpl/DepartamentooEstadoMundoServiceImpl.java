...
import com.backendsicimcede10ejcnacv10.mil.co.backendsicimcede10ejcnacv10.persistencia.dao.DepartamentooEstadoMundoDAO;
...
public class DepartamentooEstadoMundoServiceImpl implements DepartamentooEstadoMundoService {
    ...
    public Slice<DepartamentooEstadoMundoDTO> listarDepartamentosoEstadosMundoPag(Pageable pageable, Long idPaisMundo, String nombrePaisMundo, String keyword, String orderBy, String orderMode) {
        return departamentooEstadoMundoRepository.findAllDepartamentosoEstadosMundoPag(pageable, idPaisMundo, nombrePaisMundo, keyword, orderBy, orderMode).map(departamentooEstadoMundo -> departamentooEstadoMundoDAO.departamentooEstadoMundoDTO(departamentooEstadoMundo));
    }
    ...
    public RespuestaDTO crearDepartamentooEstadoMundo(DepartamentooEstadoMundoDTO departamentooEstadoMundoDTO) {
        ...
        DepartamentooEstadoMundo departamentooEstadoMundoNombre = departamentooEstadoMundoRepository.findByNombrePaisMundoAndNombreDepartamentooEstadoMundo(departamentooEstadoMundoDTO.getPaisMundoDTO().getNombrePaisMundo(), departamentooEstadoMundoDTO.getNombreDepartamentooEstadoMundo().toUpperCase());
        RespuestaDTO respuestaDTO = new RespuestaDTO(MensajesConstantes.MSG_REGISTRO_NO_CREADO, false);
        
        //DECLARACIÓN E INICIALIZACIÓN DE LAS BANDERAS EN CERO (0):
        long banderaNombreRegistroEncontrado=0;
        
        if (!(departamentooEstadoMundoNombre==null)) {//SI ENCONTRO EL NOMBRE DEL REGISTRO EN LA TABLA DE LA BASE DE DATOS Y ASOCIADO A UN MISMO REGISTRO RELACIONAL MUESTRA UN MENSAJE DE NOMBRE DE REGISTRO REPETIDO CON EL NOMBRE PROPORCIONADO.
    ...
    //LEER CONSULTA DE REGISTRO POR NOMBRE DENTRO DE PAIS DEL MUNDO:
    @Override//SOBREESCRIBIMOS EL METODO DE LEER CONSULTA DE REGISTRO.
    public RespuestaDTO consultarDepartamentooEstadoMundoporIdeIdPaisMundo(Long idPaisMundo, Long idDepartamentooEstadoMundo) {
        Optional<DepartamentooEstadoMundo> departamentooEstadoMundoIdeIdPaisMundo = Optional.ofNullable(departamentooEstadoMundoRepository.findByIdPaisMundoAndIdDepartamentooEstadoMundo(idPaisMundo, idDepartamentooEstadoMundo));
        RespuestaDTO respuestaDTO = new RespuestaDTO(MensajesConstantes.MSG_REGISTRO_ID_NO_ENCONTRADO, false);
        
        if (departamentooEstadoMundoIdeIdPaisMundo.isPresent()==true) {//SI ENCONTRO EL ID DEL REGISTRO EN LA TABLA DE LA BASE DE DATOS MUESTRA EL REGISTRO CON UN MENSAJE DE CONSULTA EXITOSA CON EL ID E ID PROPORCIONADOS.
           respuestaDTO.setDepartamentooEstadoMundoDTO(departamentooEstadoMundoDAO.departamentooEstadoMundoDTO(departamentooEstadoMundoIdeIdPaisMundo.get()));
           respuestaDTO.setMensaje(MensajesConstantes.MSG_REGISTRO_CONSULTADO_EXITO);
           respuestaDTO.setBanderaexito(true);
        }
        if (departamentooEstadoMundoIdeIdPaisMundo.isPresent()==false) {//SI NO ENCONTRO EL ID DEL REGISTRO EN LA TABLA DE LA BASE DE DATOS MUESTRA EL REGISTRO CON UN MENSAJE DE CONSULTA NO EXITOSA CON EL ID E ID PROPORCIONADOS.
           respuestaDTO = new RespuestaDTO(MensajesConstantes.MSG_REGISTRO_ID_NO_ENCONTRADO, false);
           respuestaDTO.setDepartamentooEstadoMundoDTO(null);
        }
    ...
    public RespuestaDTO consultarDepartamentooEstadoMundoporNombreyNombrePaisMundo(String nombrePaisMundo, String nombreDepartamentooEstadoMundo) {
        Optional<DepartamentooEstadoMundo> departamentooEstadoMundoNombreyNombrePaisMundo = Optional.ofNullable(departamentooEstadoMundoRepository.findByNombrePaisMundoAndNombreDepartamentooEstadoMundo(nombrePaisMundo, nombreDepartamentooEstadoMundo));
        RespuestaDTO respuestaDTO = new RespuestaDTO(MensajesConstantes.MSG_REGISTRO_NOMBRE_NO_ENCONTRADO, false);
        
        if (departamentooEstadoMundoNombreyNombrePaisMundo.isPresent()==true) {//SI ENCONTRO EL NOMBRE DEL REGISTRO EN LA TABLA DE LA BASE DE DATOS MUESTRA EL REGISTRO CON UN MENSAJE DE CONSULTA EXITOSA CON EL NOMBRE Y NOMBRE PROPORCIONADOS.
           respuestaDTO.setDepartamentooEstadoMundoDTO(departamentooEstadoMundoDAO.departamentooEstadoMundoDTO(departamentooEstadoMundoNombreyNombrePaisMundo.get()));
           respuestaDTO.setMensaje(MensajesConstantes.MSG_REGISTRO_CONSULTADO_EXITO);
           respuestaDTO.setBanderaexito(true);
        }
        if (departamentooEstadoMundoNombreyNombrePaisMundo.isPresent()==false) {//SI NO ENCONTRO EL NOMBRE DEL REGISTRO EN LA TABLA DE LA BASE DE DATOS MUESTRA EL REGISTRO CON UN MENSAJE DE CONSULTA NO EXITOSA CON EL ID Y NOMBRE PROPORCIONADOS.
